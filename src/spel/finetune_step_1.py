...
Running this script will automatically download nlwiki_step1.tar.gz (3.0 GBs)
 ...
 As well, the big tar.gz data file will not be extracted on your disc, so you do not need to reserve more than 3.0 GBs
...
ENABLE_AMP = True  # enable mixed precision gradient computation
...
def ddp_setup(rank, _world_size_):
    ...
    init_process_group(backend="nccl", rank=rank, world_size=_world_size_) #gloo


def get_mode_checkpoint_name():
    return os.path.join(get_checkpoints_dir(), "nlspel-step-1-ddp")
...
def finetune_step_1(rank: int, world_size: int, n_epochs, batch_size, eval_batch_size=32, encoder_lr=5e-5,
    ...
    if continue_from_previous_checkpoint:
        model.load_checkpoint('nlspel-step-1-ddp.pt', device=device, rank=rank)
    if TRACK_WITH_WANDB and rank == 0:
        wandb.init(
            project="nlspel-finetune-step-1-ddp",
            config={
                "n_epochs": n_epochs,
    ...
    for epoch in range(n_epochs):
        ...
        created_dataset = get_dataset(
            dataset_name='nlwiki', get_labels_with_high_model_score=model.get_highest_confidence_model_predictions,
        ...
        for iter_, (inputs, subword_mentions) in _iter_:
            if iter_ >= 39306:
                print("Breaking after 39306 iterations")
                break
            token_ids = inputs.token_ids.to(device)
            mention_ids = subword_mentions.ids.to(device)
            mention_probs = subword_mentions.probs.to(device)
            if token_ids.is_sparse:
                token_ids = token_ids.to_dense()
            if mention_ids.is_sparse:
                mention_ids = mention_ids.to_dense()
            if mention_probs.is_sparse:
                mention_probs = mention_probs.to_dense()

            if ENABLE_AMP:
                with torch.cuda.amp.autocast():
                    logits = model.get_model_raw_logits_training(
                        token_ids, mention_ids, mention_probs)
                    logits = logits.view(-1)
                    label_probs = mention_probs.view(-1)
            ...
            if (iter_ + 1) % evaluate_every == 0 and rank == 0:
                ...
                print(f"Best f1 per epoch: {best_f1_per_epoch}") # debugging
...
if __name__ == "__main__":
    try:
        ...
        _arguments_ = {
            ...
            "encoder_lr": 5e-5,
            "decoder_lr": 0.01,
            "accumulate_batch_gradients": 4, #4
            "label_size": 10240, #10240
            "evaluate_every": int(4000/w_size) if w_size > 1 else 15000,
            "n_epochs_freeze_bert": 3, # 3
            "early_stopping_n_epoch_wait": 3,
        ...
        print("Starting the training processes ...")
