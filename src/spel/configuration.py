...
MULTINERD_CANONICAL_REDIRECTS = None
...
def get_multinerd_train_canonical_redirects():
    global MULTINERD_CANONICAL_REDIRECTS
    if not MULTINERD_CANONICAL_REDIRECTS:
        r_file = get_resources_dir() / "data" / "multinerd_canonical_redirects.json"
        with r_file.open() as f:
            MULTINERD_CANONICAL_REDIRECTS = json.load(f)
    return MULTINERD_CANONICAL_REDIRECTS
...
def get_multinerd_yago_tsv_file_path():
    return get_resources_dir() / "data" / "MULTINERD-dataset.tsv"
...
def get_multinerd_vocab():
    mentions_vocab = dict({'|||O|||': 0, '<pad>': 1})
    dictionary_file = get_resources_dir() / "vocab" / "multinerd.txt"
...
def get_multinerd_plus_wikipedia_vocab():
    mentions_vocab = get_multinerd_vocab()
    dictionary_file = get_resources_dir() / "vocab" / f"nlwiki_20250320.txt"
    dfile = dictionary_file.open("r")
    for _ad_element in dfile.read().split("\n"):
...
def get_multinerd_plus_wikipedia_plus_out_of_domain_vocab():
    mentions_vocab = get_multinerd_plus_wikipedia_vocab()
