...
from spel.multinerd import MULTINERDDataset
from spel.configuration import get_resources_dir
from mosestokenizer import MosesTokenizer, MosesPunctuationNormalizer
moses_tokenize = MosesTokenizer('nl', old_version=True)
normalize = MosesPunctuationNormalizer('nl')

def convert_tokens_to_string_keep_characters(tokens):
    return "".join(tokens).replace("Ġ", " ")

def fix_misencodings(text):
    replacements = {
        'Ã«': 'ë', 'Ã©': 'é', 'Ã¨': 'è', 'Ãª': 'ê', 'ÄĽ': 'ě', 'Ã¢': 'â', 'Ã£': 'ã', 'Ã½': 'ý',
        'Ãº': 'ú', 'Ã¯': 'ï', 'Ãħ': 'Å', 'Ã³': 'ó', 'Ã²': 'ò', 'Ãĸ': 'Ö', 'Ã¶': 'ö', 'ÃŃ': 'í',
        'Ã¬': 'ì', 'Åĳ': 'ő', 'Ã¸': 'ø', 'Äħ': 'ą', 'Ã¡': 'á', 'Ãł': 'à', 'Å¡': 'š', 'Ã´': 'ô',
        'Ãİ': 'Î', 'Ãī': 'É', 'Ãģ': 'Á', 'Ã§': 'ç', 'Ãĺ': 'Ø', 'Äĩ': 'ć', 'Ã¤': 'ä', 'Ã°': 'ð',
        'Ä±': 'ı','ÃŁ': 'ß', 'Å¾': 'ž', 'ÄĨ': 'Ć', 'Ã¥': 'å', 'Ã¦': 'æ', 'âĢĻ': '’', 'Åł': 'Š',
        'Ã¼': 'ü', 'ÅĤ': 'ł', 'ÈĽ': 'ț', 'ÅĦ': 'ń', 'Äį': 'č', 'Ã¥': 'å', 'Åŀ': 'Ş', 'Ã±': 'ñ',
        'Ãĺ': 'Ø', 'Ã»': 'û', 'ÈĻ': 'ș', 'Äģ': 'ā', 'Ã®': 'î', 'ÅĨ': 'ņ', 'ÅĽ': 'ś', 'Ä°': 'İ',
        'Ãļ': 'Ú', 'ÅŁ': 'ş', 'Ã¿': 'ÿ', 'á»ĵ': 'ồ', 'Å½': 'Ž', 'ÅĪ': 'ň', 'Äĳ': 'đ', 'Âµ': 'µ',
        'á¹Ł': 'ṟ', 'ÄĻ': 'ę', 'ÄĮ': 'Č', 'á»£': 'ợ', 'âĢĺ': '‘', 'Äĥ': 'ă', 'á¹£': 'ṣ', 'ÅĻ': 'ř',
        'á»Ľ': 'ớ', 'Åļ': 'Ś', '£á¹ĩ': 'ṇ', '£á¹': 'ṇ', 'á¹ĩ': 'ṇ', 'Ã¹': 'ù', 'Ä¡': 'ġ', 'ÄĲ': 'Đ',
        'Åĵ': 'œ', 'Å«': 'ū', 'ÄĹ': 'ė', 'á»ĩ': 'ệ', 'áº¡': 'ạ', 'á»¥': 'ụ', 'áº¡': 'ạ', 'Æ¡': 'ơ',
        'á»Ļ': 'ộ', 'Å©': 'ũ', 'áº¿': 'ế', 'ÄŁ': 'ğ', 'Äł': 'G', 'áºŃ': 'ậ', 'áº£': 'ả', 'Ä«': 'ī',
        'ÃĦ': 'Ä', 'Â°': '°', 'Åģ': 'Ł', 'Åº': 'ź', 'Ãµ': 'õ'
    }
    for wrong, right in replacements.items():
        text = text.replace(wrong, right)
    return text

def fix_problematic_cases(sentence):
    pattern = r"\((\d+)\)"
    sentence = re.sub(pattern, r"( \1 )", sentence)
    sentence = re.sub(r'"\( 25924 \) Douglasadams"', r'" ( 25924 ) Douglasadams "', sentence)
    sentence = re.sub(r'\("Allium"\)', r'( " Allium " )', sentence)
    sentence = re.sub(r'\("Corynephorus canescens"\)', r'( " Corynephorus canescens " )', sentence)
    sentence = re.sub(r'\("Cavia porcellus"\)', r'( " Cavia porcellus " )', sentence)
    sentence =  re.sub(r"'Somethin'", r"' Somethin '", sentence)
    sentence =  re.sub(r"'cellulitis'", r"' cellulitis '", sentence)
    sentence =  re.sub(r"\(kever\)", r"( kever )", sentence)
    sentence =  re.sub(r"\(1929 -\)", r"( 1929 - )", sentence)
    sentence = re.sub(r"Ġ", "G", sentence)
    return sentence
...
def get_subword_to_word_mapping(subword_tokens, original_string, sequence_starts_and_ends_with_bos_eos=True):
    ...
    while len(subword_to_word_mapping) != len(original_tokens):
        next_t = convert_tokens_to_string_keep_characters(subword_tokens[start_subword_index:end_subword_index])
        next_t = next_t.strip()
        ...
        if end_subword_index - start_subword_index > 1000:
            for i in [0, 1, 2, 3, 4]:
                n = convert_tokens_to_string_keep_characters(subword_tokens[start_subword_index:start_subword_index + 2 + i]).strip()
...
def store_validation_data_wiki(checkpoints_root, batch_size, label_size, is_training, use_retokenized_wikipedia_data):
    dataset_name = f"validation_data_cache_b_{batch_size}_l_{label_size}_" \
                   f"{('rt_wiki' if use_retokenized_wikipedia_data else 'wiki') if is_training else 'mn'}/"
    ...
    if is_training:
        valid_iter = tqdm(get_dataset(
            dataset_name='nlwiki', split='valid', batch_size=batch_size, label_size=label_size,
            use_retokenized_wikipedia_data=use_retokenized_wikipedia_data))
    else:
        valid_iter = tqdm(get_dataset(dataset_name='multinerd', split='valid', batch_size=batch_size, label_size=label_size))
...
def calculate_offsets(original_string, final_tokens):
    offsets = []
    i = 0  # pointer into original string
    
    for token in final_tokens:
        while i < len(original_string) and original_string[i].isspace():
            i += 1
        clean_token = token.replace("Ġ", "G")

        start = i
        end = start + len(clean_token)

        if original_string[start:end] != clean_token:
            found = original_string.find(clean_token, i)
            if found == -1:
                raise ValueError(f"Token '{clean_token}' not found in original string at position {i}")
                # print(f"{clean_token}{original_string}")
            start, end = found, found + len(clean_token)
            i = end
        else:
            i = end
...
def merge_tokenized_mention(tokenized_mentions, original_string):
    # Join tokens and fix misencodings

    tokens_merged_bos_eos = fix_misencodings(
        ''.join(tokenized_mentions.tokens())
        .replace("Ġ", " Ġ")
        .replace("<s>", "<s> ")
        .replace("</s>", " </s>")
        .replace("(", " ( ")
    ...
    # Remove <s> and </s> for processing
    tokens_merged = tokens_merged_bos_eos[1:-1]

    final_tokens = []
    for i, token in enumerate(tokens_merged):
        if token == '(':
            final_tokens.append("Ġ(")
            continue
        elif token == ')':
            final_tokens.append("Ġ)")
            continue
            ...
            continue
        elif token.startswith('Ġ"') and len(token) > 2 and token.endswith('"'):
            final_tokens.append("Ġ" + token[1])
            final_tokens.append("Ġ" + token[2:-1])
            final_tokens.append("Ġ" + token[-1])
            continue
        if not token:
            continue
        ...
        if token[0] not in string.punctuation and token[-1] not in string.punctuation:
            if token.startswith("Ġ'") and len(token) > 2:
                final_tokens.append("Ġ" + token[1])
                final_tokens.append("Ġ" + token[2:])
            elif token.startswith('Ġ\"') and len(token) > 2:
                final_tokens.append("Ġ" + token[1])
                final_tokens.append("Ġ" + token[2:])
        ...
        # Case: leading/trailing punctuation
        before_tokens = []
        after_tokens = []
        while token and token[0] in string.punctuation:
            before_tokens.append(token[0])
            token = token[1:]
        while token and token[-1] in string.punctuation:
            after_tokens.append(token[-1])
            token = token[:-1]
        if before_tokens:
            final_tokens.append("".join(before_tokens))
        if token:
            final_tokens.append(token)
        if after_tokens:
            final_tokens.append("".join(after_tokens[::-1]))

    final_tokens = ["<s>"] + [token for token in final_tokens if token != "Ġ"] + ["</s>"]
    
    final_tokens_unprocessed = final_tokens.copy()
    for i,token in enumerate(final_tokens_unprocessed):
        if i > 0 and token[0] != "Ġ":
            final_tokens_unprocessed[i] = f"Ġ{token}"
    ...
    try:
        ...
        for i, token in enumerate(final_tokens):
            ...
            if token == "\"":
                final_tokens[i] = "Ġ\""
                if i + 1 < len(final_tokens) and final_tokens[i + 1] == "Ġ(":
                    opened += 1
                    replace_open.append(i + 1)
                elif final_tokens[i - 1] == "Ġ)":
                    opened += 1
                    replace_close.append(i - 1)
        
        if len(replace_close) > 0 and len(replace_open) > 0:
            if opened % 2 != 0:
                opened = opened - 1
            for i in replace_close[:opened//2]:
                final_tokens[i] = "Ġ)\""
                final_tokens[i + 1] = "RM"
            for i in replace_open[:opened//2]:
                final_tokens[i] = "Ġ\"("
                final_tokens[i - 1] = "RM"

        final_tokens = [token for token in final_tokens if token != "RM"]
        for i,token in enumerate(final_tokens):
            if i > 0 and token[0] != "Ġ":
                final_tokens[i] = f"Ġ{token}"

        # Compute offsets
        pattern = r'"\s*\(\s*(.*?)\s*\)\s*"'
        replacement = r'"( \1 )"'
        original_string = re.sub(pattern, replacement, original_string)
        pattern = r"\((\d+)\)"
        original_string = re.sub(pattern, r"( \1 )", original_string)
        tokens_merged_offsets = calculate_offsets(original_string, final_tokens[1:-1])


    except ValueError:
        # Compute offsets
        tokens_merged_offsets = calculate_offsets(original_string, final_tokens_unprocessed[1:-1])
    return final_tokens, tokens_merged_offsets, original_string
...
def get_multinerd_set_phrase_splitted_documents(dataset_name):
    d_iter = MULTINERDDataset().dataset[dataset_name]
    ...
    for document in d_iter:
        ...
        for annotation in document.annotations:
            for a in annotation:
                token_str = str(a.token).strip()
                document_words.append(token_str)

                document_candidates.append([x.url.replace('http://nl.wikipedia.org/wiki/', '')
                                            for x in a.candidates.candidates] if a.candidates else [])
                if a.mn_entity and a.mn_entity != "--NME--":
                    # document_labels.append(a.yago_entity.encode('ascii').decode('unicode-escape'))
                    document_labels.append(a.mn_entity)
                else:
                    document_labels.append('|||O|||')
        original_string = " ".join(document_words)
        tokenized_mention = tokenizer(original_string)

        tokens_merged, tokens_merged_offsets, original_string = merge_tokenized_mention(tokenized_mention, original_string)
        original_string = fix_problematic_cases(original_string)
        document_words = original_string.split()
        # tokens_offsets = list(zip(tokenized_mention.tokens(), tokenized_mention.encodings[0].offsets))[1:-1]
        tokens_offsets = list(zip(tokens_merged, tokens_merged_offsets))[1:-1]
        # mapping = get_subword_to_word_mapping(tokenized_mention.tokens(), original_string)
        mapping = get_subword_to_word_mapping(tokens_merged, original_string)
        # subword_tokens = tokenized_mention.tokens()[1:-1]
        subword_tokens = tokens_merged[1:-1]
        ...
        for w, l, cnds in zip(document_words, document_labels, document_candidates):
            ...
            if w == converted_to_words:
                ...
                print("Word:", w)
                print("Representation:", repr(w))
                print("Converted subwords:", [x for x in subword_tokens[mapping[w_ind][0]:mapping[w_ind][1]]])
...
def chunk_annotate_and_merge_to_phrase(model, sentence, k_for_top_k_to_keep=5, normalize_for_chinese_characters=False):
    sentence = fix_problematic_cases(sentence)
    sentence = sentence.rstrip()

    sentence = fix_misencodings(sentence)
    ...
    tokens_merged, tokens_merged_offsets, original_string =  merge_tokenized_mention(tokenized_mention, sentence)

    # tokens_offsets = list(zip(tokenized_mention.tokens(), tokenized_mention.encodings[0].offsets))
    tokens_offsets = list(zip(tokens_merged, tokens_merged_offsets))
    # subword_to_word_mapping = get_subword_to_word_mapping(tokenized_mention.tokens(), sentence)
    subword_to_word_mapping = get_subword_to_word_mapping(tokens_merged, sentence)
    ...
    word_annotations = [w for w in word_annotations if w.word_string != ""]

    for w in word_annotations:
        w.word_string = fix_misencodings(w.word_string)
...
def compare_gold_and_predicted_annotation_documents(gold_document, predicted_document, ignore_over_generated=False,
    ...
    if ignore_predictions_outside_candidate_list:
        c_results = []
        for g, p, r in comparison_results:
            g_multinerd_candidates = [dl_sa.mentions_vocab[x] for x in g.multinerd_candidates if x in dl_sa.mentions_vocab]
            if g_multinerd_candidates:
                all_p_anns = p.all_possible_annotations()
                filtered_p_predictions = sorted([x for x in all_p_anns if x[0] in g_multinerd_candidates],
