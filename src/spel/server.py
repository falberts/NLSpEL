...
Note: this code has not been tested for NLSpEL, and has only been left in to potentially adapt to work in the future.
...
cli.show_server_banner = lambda *x: click.echo(
    " * You may perform entity linking through: http://localhost:3002/annotate_[multinerd,wiki,dbpedia,n3]")
...
if candidate_setting != "n":
    candidates_manager_to_use = CandidateManager(dl_sa.mentions_vocab,
                                                 ...
                                                 is_multinerd_c=candidate_setting.startswith("p"),
...
def generic_annotate(nif_bytes, load_multinerd_finetuned, kb_prefix, load_full_vocabulary=False):
    ...
    if gerbil_communication_done:
        ...
        with lock:
            annotator.annotate(parsed_collection, ignore_non_multinerd_vocab=not load_full_vocabulary, kb_prefix=kb_prefix,
        ...
        if load_multinerd_finetuned and not load_full_vocabulary:
            annotator.shrink_classification_head_to_multinerd(device=device)
            annotator.load_checkpoint(None, device=device, load_from_torch_hub=True, finetuned_after_step=3)
        elif load_multinerd_finetuned:
...
@app.route('/annotate_multinerd', methods=['GET', 'POST', 'OPTIONS'])
@cross_origin(origins='*')
def annotate_multinerd():
    """Use this API for MULTINERD dataset."""
