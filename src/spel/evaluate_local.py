...
class SpELEvaluator(SpELAnnotator):
    ...
    def wikipedia_evaluate(self, checkpoint_name, use_retokenized_wikipedia_data=False):
        self.init_model_from_scratch(device=device)
        if checkpoint_name is None:
            print(f'Loading the model which is not fine-tuned on MULTINERD/CoNLL dataset '
                  f'(retokenized={use_retokenized_wikipedia_data}) ...')
            self.load_checkpoint(None, device=device, load_from_torch_hub=False,
...
if __name__ == '__main__':
    ...
    b_annotator.wikipedia_evaluate(checkpoint_name=BERT_ANNOTATOR_CHECKPOINT, use_retokenized_wikipedia_data=False)
