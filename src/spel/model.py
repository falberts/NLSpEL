...
from spel.utils import store_validation_data_wiki, chunk_annotate_and_merge_to_phrase, \
    ...
    convert_tokens_to_string_keep_characters, fix_problematic_cases
...
from spel.configuration import get_checkpoints_dir, get_multinerd_train_canonical_redirects, get_ood_canonical_redirects, \
...
class SpELAnnotator:
    ...
    def init_model_from_scratch(self, base_model=BERT_MODEL_NAME, device="cpu"):
        ...
        if base_model:
            ...
            self.out = nn.Embedding(num_embeddings=len(dl_sa.mentions_vocab),
                        embedding_dim=self.bert_lm_h, sparse=False).to(device) # sparse=False for AMP support
            self.softmax = nn.Softmax(dim=-1)

    def shrink_classification_head_to_multinerd(self, device):
        """
        This can be called in fine-tuning step 2 to shrink the classification head to in-domain data vocabulary. Currently this does not work.
        """
        multinerd_mentions_vocab, multinerd_mentions_itos = dl_sa.get_multinerd_vocab_and_itos()
        if self.out_module.num_embeddings == len(multinerd_mentions_vocab):
            return
        current_state_dict = self.out_module.state_dict()
        new_out = nn.Embedding(num_embeddings=len(multinerd_mentions_vocab),
                               embedding_dim=self.bert_lm_h, sparse=False).to(device)
        new_state_dict = new_out.state_dict()
        for index_new in range(len(multinerd_mentions_itos)):
            item_new = multinerd_mentions_itos[index_new]
            assert item_new in dl_sa.mentions_vocab, \
                "the multinerd fine-tuned mention vocab must be a subset of the original vocab"
        ...
        dl_sa.shrink_vocab_to_multinerd()
    ...
    @staticmethod
    def get_canonical_redirects(limit_to_mn=True):
        return get_multinerd_train_canonical_redirects() if limit_to_mn else get_ood_canonical_redirects()
    ...
    def create_optimizers(self, encoder_lr=5e-5, decoder_lr=0.1, exclude_parameter_names_regex=None):
        ...
        if decoder_lr < 1e-323:
            ...
            decoder_lr = 1e-323
        # out_optim = optim.SparseAdam(self.out.parameters(), lr=decoder_lr) # When not using AMP
        out_optim = optim.Adam(self.out.parameters(), lr=decoder_lr)
    ...
    def get_model_raw_logits_training(self, token_ids, label_ids, label_probs):
        ...
        # Ensure enc and out are CUDA and dense
        if enc.is_sparse:
            enc = enc.to_dense()
        if out.is_sparse:
            out = out.to_dense()
    ...
    def inference_evaluate(self, epoch, best_f1, dataset_name='testa'):
        ...
        print(f"len(gold_documents): {len(gold_documents)}")

        if not gold_documents:
            print(f"No gold documents found for dataset {dataset_name}. Skipping inference evaluation.")
            return evaluation_results
        for gold_document in tqdm(gold_documents):
            # print(f"Gold Document: {[x.word_string for x in gold_document]}")
            t_sentence = " ".join([x.word_string for x in gold_document])
            t_sentence = fix_problematic_cases(t_sentence)
            # print(f"Tokenized Sentence: {t_sentence}")
            ...
            # print("Gold entity-level spans:", g_el)
            # print("Predicted entity-level spans:", p_el)

            if p_el:
                evaluation_results.record_mention_detection_results(p_md, g_md)
                evaluation_results.record_entity_linking_results(p_el, g_el)

        f1_new = evaluation_results.micro_entity_linking.f1.compute()
    ...
    def download_large_from_torch_hub(finetuned_after_step=1):
        """
        Downloads checkpoints from HF hub. Note: this applies to the original SpEL checkpoints, 
        not the ones created for NLSpEL, and as a result may not work with this implementation.
        """
        assert 4 >= finetuned_after_step >= 1
        if finetuned_after_step == 4:
            # This model is the same SpEL finetuned model after step 3 except that its classification layer projects to
    ...
    def load_checkpoint(self, checkpoint_name, device="cpu", rank=0, load_from_torch_hub=False, finetuned_after_step=1):
        if load_from_torch_hub and BERT_MODEL_NAME == "pdelobelle/robbert-v2-dutch-ner":
            checkpoint = self.download_large_from_torch_hub(finetuned_after_step)
            self._load_from_checkpoint_object(checkpoint, device)
        elif load_from_torch_hub and BERT_MODEL_NAME == "pdelobelle/robbert-v2-dutch-ner":
