...
class CandidateManager:
    def __init__(self, mentions_vocab, is_kb_yago = False, is_multinerd_c = False, is_context_agnostic = False,
        ...
        if is_kb_yago:
            ...
            self.load_multinerd_candidates(is_context_agnostic, is_indexed_for_spans)
        else:
            raise ValueError("Either \'is_kb_yago\' or \'is_multinerd_c\' flags must be True!")
        self.is_context_agnostic = is_context_agnostic
        self.is_indexed_for_spans = is_indexed_for_spans
        self.is_kb_yago = is_kb_yago
        self.is_multinerd_c = is_multinerd_c

    def load_multinerd_candidates(self, is_context_agnostic, is_indexed_for_spans):
        ...
        candidates_a = json.load(open(
            get_resources_dir() / "data" / "candidates" / "multinerd_testa_pprforned" / file_address, "r"))
        candidates_b = json.load(open(
            get_resources_dir() / "data" / "candidates" / "multinerd_testb_pprforned" / file_address, "r"))
        if is_context_agnostic:
