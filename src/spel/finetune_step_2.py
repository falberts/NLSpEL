...
Running this script will automatically download multinerd.tar.gz (30.9 MBs)
 ...
 format: validation_data_cache_b_<batch_size>_l_<label_size>_mn. You do not need to worry about downloading or
...
class FinetuneS2(SpELAnnotator):
    ...
    def finetune(self, checkpoint_name, n_epochs, batch_size, bert_dropout=0.2, encoder_lr=5e-4, label_size=8196,
        ...
        if checkpoint_name is None:
            ...
            checkpoint_name = 'nlwiki_finetuned_step_1_model_checkpoint'
        else:
        ...
        for epoch in range(n_epochs):
            ...
            _iter_ = tqdm(enumerate(
                get_dataset(dataset_name='multinerd', split='train', batch_size=batch_size, label_size=label_size,
            ...
            sprecision, srecall, sf1, sf05, snum_proposed, snum_correct, snum_gold, subword_eval = self.evaluate(
                epoch, eval_batch_size, self.out_module.num_embeddings, 1.1, is_training=True
            )
    ...
    def get_mode_checkpoint_name(self):
        return os.path.join(self.checkpoints_root, "nlspel-step-2")


if __name__ == '__main__':
    try:
        b_annotator = FinetuneS2()
        checkpoint_name = 'nlspel-step-1-ddp.pt'
        b_annotator.finetune(checkpoint_name=checkpoint_name, n_epochs=60, batch_size=10, bert_dropout=0.2, label_size=10240,
                             eval_batch_size=2)
    finally:
        if TRACK_WITH_WANDB:
            wandb.finish()
