...
class CreateResolveToWikiNameDicts(PipelineJob):
    ...
    def __init__(self, preprocess_jobs: Dict[str, PipelineJob], opts):
        super().__init__(
            requires=["data/indexes/redirects_lang=nl.ttl.bz2.dict"],
            provides=[
                "data/indexes/freebase-links_lang=nl.ttl.bz2.dict",
                "data/downloads/freebase-links_lang=nl.ttl.bz2",
                "data/indexes/page_lang=nl_ids.ttl.bz2.dict",
                "data/downloads/page_lang=nl_ids.ttl.bz2",
            ],
    ...
    def _run(self):
        ...
        fb_to_wikiname_dict = self._create_dict(
            redirects=redirects,
            url="https://downloads.dbpedia.org/repo/lts/transition/freebase-links/2019.02.10/freebase-links_lang=nl.ttl.bz2",
            matcher_pattern="<http://nl.dbpedia.org/resource/(.*)> <http://www.w3.org/2002/07/owl#sameAs> <http://rdf.freebase.com/ns/(.*)> .",
            postproc_key=lambda x: "/" + x.replace(".", "/"),
        )
        with open("data/indexes/freebase-links_lang=nl.ttl.bz2.dict", "wb") as f:
            pickle.dump(fb_to_wikiname_dict, f)

        page_id_to_wikiname_dict = self._create_dict(
            redirects=redirects,
            url="https://downloads.dbpedia.org/repo/dbpedia/generic/page/2022.12.01/page_lang=nl_ids.ttl.bz2",
            matcher_pattern='<http://dbpedia.org/resource/(.*)> <http://dbpedia.org/ontology/wikiPageID> "(.*)"\^\^<http://www.w3.org/2001/XMLSchema#integer> .',
        )
        with open("data/indexes/page_lang=nl_ids.ttl.bz2.dict", "wb") as f:
