...
class CollectMentionEntityCounts(PipelineJob):
    ...
    def _run(self):
        ...
        self.log("Load ./data/indexes/redirects_lang=nl.ttl.bz2.dict")
        with open(f"data/indexes/redirects_lang=nl.ttl.bz2.dict", "rb") as f:
        ...
        for id in range(self.opts.collect_mention_entities_num_workers):
            worker = Worker(in_queue, out_queue, redirects_nl)
            worker.start()
        ...
        for file_nr, extracted_wiki_file in enumerate(tqdm(glob.glob(list_dir_string))):
            try:
                ...
                print(f"Timeout waiting for output from worker at file {extracted_wiki_file}")
...
class Worker(multiprocessing.Process):
    def __init__(self, in_queue, out_queue, redirects_nl):
        super().__init__()
        self.in_queue = in_queue
        self.out_queue = out_queue
        self.redirects_nl = redirects_nl
