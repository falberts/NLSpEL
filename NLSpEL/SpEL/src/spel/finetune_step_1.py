...
Running this script will automatically download nlwiki_step1.tar.gz (3.1 GBs)
    ...
    for epoch in range(n_epochs):
        ...
        for iter_, (inputs, subword_mentions) in _iter_:
            if iter_ >= 39306:
                ...
                break
            token_ids = inputs.token_ids.to_dense() if inputs.token_ids.is_sparse else inputs.token_ids
            mention_ids = subword_mentions.ids.to_dense() if subword_mentions.ids.is_sparse else subword_mentions.ids
            mention_probs = subword_mentions.probs.to_dense() if subword_mentions.probs.is_sparse else subword_mentions.probs

            token_ids = token_ids.to(device)
            mention_ids = mention_ids.to(device)
            mention_probs = mention_probs.float().to(device)
...
if __name__ == "__main__":
    try:
        ...
        _arguments_ = {
            ...
            "n_epochs_freeze_bert": 0, # 3
            "early_stopping_n_epoch_wait": 8,
