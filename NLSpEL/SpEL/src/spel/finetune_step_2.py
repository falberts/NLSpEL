...
The implementation for domain specific fine-tuning, step two. This process is very light and can run on one Nvidia
1060 with 6 GBs of GPU memory.

Running this script will automatically download multinerd.tar.gz (35.5 MBs)
...
if __name__ == '__main__':
    try:
        ...
        b_annotator.finetune(checkpoint_name=checkpoint_name, n_epochs=30, batch_size=10, bert_dropout=0.2, label_size=59049,
                             eval_batch_size=2)
    finally:
        if TRACK_WITH_WANDB:
            wandb.finish()
