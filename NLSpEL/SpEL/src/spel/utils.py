...
def unicode_normalize(s, mode="NFKC"):
...
def calculate_offsets(original_string, final_tokens):
    ...
    for token in final_tokens:
        ...
        if original_string[start:end] != token:
            found = original_string.find(token, i)
            if found == -1:
                print(f"Original string: '{original_string}'")
                print(f"Final tokens: {final_tokens}")
                raise ValueError(f"Token '{token}' not found in original string at position {i}")
            start, end = found, found + len(token)
            i = end
        else:
            i = end
...
def merge_tokenized_mention(tokenized_mention, original_string):

    raw_tokens = tokenized_mention.tokens()

    if len(raw_tokens) < 2:
        raise ValueError("Tokenizer returned too few tokens for mention.")


    tokens_merged = list(raw_tokens)

    inner_tokens = tokens_merged[1:-1]
    tokens_merged_offsets = calculate_offsets(original_string, inner_tokens)

    return tokens_merged, tokens_merged_offsets, original_string
    ...
    for document in d_iter:
        ...
        for annotation in document.annotations:
            for a in annotation:
                ...
                document_bio_tags.append(a.begin_inside_tag if a.begin_inside_tag else 'O')
        
        original_string = unicode_normalize(" ".join(document_words))
        ...
        for w, l, cnds, bio in zip(document_words, document_labels, document_candidates, document_bio_tags):
            l = unicode_normalize(l)
            converted_to_words = "".join([x[1:] if x.startswith("▁")
            ...
            if w == converted_to_words:
                ...
                wa = WordAnnotation(subword_annotations[mapping[w_ind][0]:mapping[w_ind][1]],
                                    tokens_offsets[mapping[w_ind][0]:mapping[w_ind][1]], cnds)
                wa.begin_inside_tag = bio
                word_annotations.append(wa)
                w_ind += 1
            elif len(mapping) > w_ind + 1 and w == "".join([x[1:] if x.startswith("▁")
                ...
                for sub_w in subword_tokens[mapping[w_ind][0]:mapping[w_ind+1][1]]:
                    subword_annotations.append(SubwordAnnotation([1.0], [dl_sa.mentions_vocab[l]], sub_w))
                wa = WordAnnotation(subword_annotations[mapping[w_ind][0]:mapping[w_ind+1][1]],
        ...
        for w in word_annotations:
            if phrase_annotations and phrase_annotations[-1].resolved_annotation == w.resolved_annotation:
                ...
                phrase_annotations[-1].begin_inside_tag = phrase_annotations[-1].words[0].begin_inside_tag
            else:
                p = PhraseAnnotation(w)
                p.begin_inside_tag = w.begin_inside_tag
