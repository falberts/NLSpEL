...
class WordAnnotation:
    def __init__(self, subword_annotations, token_offsets, ppr_for_ned_candidates=None):
        ...
        self.word_string = ''.join([x[0].replace('▁', '') for x in token_offsets])
        ...
        if not self.is_valid_annotation:
            ...
            # Use union of top-k candidate lists across subwords (less conservative).
            union_ids = set().union(*[set(y.top_k_i_list) for y in self.annotations])
            cand_list = [(cid, self._get_assigned_probabilities(cid)) for cid in union_ids]
            self.candidates = sorted(cand_list, key=lambda x: sum(x[1]) / len(x[1]), reverse=True)
...
class SubwordAnnotation:
    ...
    def __init__(self, top_k_p_list, top_k_i_list, subword_string):
        ...
        self.subword_string = subword_string.replace('▁', '')
        self.bioes_label = 2
