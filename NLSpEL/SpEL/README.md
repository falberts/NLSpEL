...
---
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/spel-structured-prediction-for-entity-linking/entity-linking-on-multinerd-mn)](https://paperswithcode.com/sota/entity-linking-on-multinerd-mn?p=spel-structured-prediction-for-entity-linking)
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/spel-structured-prediction-for-entity-linking/entity-linking-on-multinerd-testc)](https://paperswithcode.com/sota/entity-linking-on-multinerd-testc?p=spel-structured-prediction-for-entity-linking)
...
[MULTINERD benchmark](https://www.mpi-inf.mpg.de/departments/databases-and-information-systems/research/ambiverse-nlu/multinerd/downloads) 
...
Entity Linking evaluation results of *SpEL* compared to that of the literature over MULTINERD test sets:
...
MULTINERD dataset:
-------------
Initially you need to prepare the MULTINERD dataset. For this:

1. Download and extract [multinerd-yago2-dataset.zip](http://resources.mpi-inf.mpg.de/yago-naga/multinerd/download/multinerd-yago2-dataset.zip).
2. Download the CoNLL-2003 data files from [mn2003/ner](https://www.cnts.ua.ac.be/mn2003/ner/) and copy/paste the generated `eng.testa`, `eng.testb`, `eng.train` files into the extracted `multinerd-yago2-dataset` directory, and run `java -jar multinerd-yago2-dataset.jar`; this will create a file named `MULTINERD-YAGO2-dataset.tsv`.
4. Place `MULTINERD-YAGO2-dataset.tsv` under `resources/data/` directory (you can find `resources` directory besides `src` directory in the main project folder).
5. A preprocessed version of this dataset will be automatically downloaded for finetuning step 3.

Note: As the `README.txt` file inside `multinerd-yago2-dataset` states:
```
...
As well, you may access the created finetuning data through the following:
   ...
   - [FT-Step3-Data (MULTINERD)](https://1sfu-my.sharepoint.com/:u:/g/personal/sshavara_sfu_ca/EajEGYyf8LBOoxqDaiPBvbgBwFuEC08nssvZwGJWsG_HXg?e=wAwV6H)
...
if finetuned_after_step == 3:
    spel.shrink_classification_head_to_multinerd(device)
...
* To replicate the results from Table 1 over the original MULTINERD test sets (`testa` and `testb`) and SpEL results of Table 
  3, you can connect GERBIL to `http://localhost:3002/annotate_multinerd`.
* To replicate the results of Table 4, as well as any experiment on `MSNBC` and our `MULTINERD/testc` experiments, 
...
4. Leave `Experiment Type` as `A2KB`, for `Matching` choose `Ma - strong annotation match`, and for `Annotator` set a preferred name (e.g. `Experiment 1`) and in `URI` set `http://localhost:3002/annotate_multinerd`.
5. Choose your evaluation `Dataset`s, for example choose `MULTINERD/CoNLL-Test A` and `MULTINERD/CoNLL-Test B` for evaluation on MULTINERD-CoNLL.
...
4,028 mentions, encompassing a total of 46,456 words. You can find this dataset under [`resources/data/multinerd_testc.ttl`](resources/data/multinerd_testc.ttl).
This dataset is in NIF format and can be easily integrated into GERBIL.

### How can I integrate MULTINERD/testc into GERBIL?
Here is the simple modifications you need to do:

1. If you are running GERBIL, stop the process.
2. Put [`resources/data/multinerd_testc.ttl`](resources/data/multinerd_testc.ttl) in `gerbil/gerbil_data/datasets/multinerd`
...
4. Copy the following lines underneath the last line defining MULTINERD/CoNLL-Test B:
   ```
   org.aksw.gerbil.datasets.MULTINERDTestC.file=${org.aksw.gerbil.DataPath}/datasets/multinerd/multinerd_testc.ttl
   org.aksw.gerbil.datasets.definition.MULTINERDTestC.name=MULTINERD/CoNLL-Test C
   org.aksw.gerbil.datasets.definition.MULTINERDTestC.class=org.aksw.gerbil.dataset.impl.nif.FileBasedNIFDataset
   org.aksw.gerbil.datasets.definition.MULTINERDTestC.cacheable=true
   org.aksw.gerbil.datasets.definition.MULTINERDTestC.experimentType=A2KB
   org.aksw.gerbil.datasets.definition.MULTINERDTestC.constructorArgs=${org.aksw.gerbil.datasets.MULTINERDTestC.file},${org.aksw.gerbil.datasets.definition.MULTINERDTestC.name}
   ```
...
## Citation
If you use SpEL finetuned models or data, gerbil_connect, or MULTINERD/testc dataset, please cite our paper:
