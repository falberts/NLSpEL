    ...
    def __init__(self, preprocess_jobs: Dict[str, PipelineJob], opts):
        super().__init__(
            requires=[
                ...
                "data/benchmarks/multinerd-dataset/MULTINERD-dataset.tsv",
            ],
            provides=[
                "data/benchmarks/multinerd-dataset/multinerd_dataset.pickle",
    ...
    def _run(self):
        ...
        subprocess.check_call(
            [
                ...
                conll2003_ner_en,
                "-C",
                "data/benchmarks/multinerd-dataset/",
            ]
        )
        ...
        try:
            subprocess.call(
                [
                    "cat data/benchmarks/multinerd-dataset/ner/etc/tags.eng"
                    " data/benchmarks/multinerd-dataset/ner/etc/tags.eng.testb"
                    " >"
                    " data/benchmarks/multinerd-dataset/ner/eng.all"
                ],
            shell=True)
        except subprocess.CalledProcessError as e:
            print(e.output)

        with open("data/benchmarks/multinerd-dataset/MULTINERD-dataset.tsv") as f1:
            merged_el = []
            el = f1.readlines()
            last_entity = None
            for el_i, el_line in enumerate(el):
                ...
                if el_line.startswith("-SPLITSTART-"):
        ...
        for line_nr, line_items in enumerate(tqdm.tqdm(merged_el)):
            ...
            if (
                len(line_items) > 0
                and line_items[0].startswith("-SPLITSTART- testa")
            ):
                split = "valid"
                print(line_items)
                print(split)
            ...
            if (
                len(line_items) > 0
                and line_items[0].startswith("-SPLITSTART- testb")
            ):
                split = "test"
                print(line_items)
                print(split)
            ...
            if len(line_items) == 1 and line_items[0] == '':
                sentence_nr += 1
                tok_nr = 0
            elif len(line_items) > 0 and line_items[0].startswith("-SPLITSTART-"):
                sentence_nr = 0
        ...
        with open("data/benchmarks/multinerd-dataset/multinerd_dataset.pickle", "wb") as f:
